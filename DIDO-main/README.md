...
## Dataset Availability and Redistribution Notice

The original datasets required by this project are **not redistributed in this repository**. Users should download the required raw IMU, ground-truth trajectory, and related sensor data directly from the corresponding official dataset sources and comply with the original licenses, terms of use, and citation requirements.

Any empty dataset directories included in this repository are placeholders showing the expected directory structure only. After downloading the original data, place the files into the corresponding directories before running preprocessing, training, or evaluation.

Any processed model outputs, corrected IMU files, or evaluation results distributed with this repository are provided solely for reproducing the experiments reported in the manuscript. They do not constitute redistribution of the original datasets and remain subject to the original datasets' applicable licenses and terms.
